        ...
        {movies.map((movie) => (
          <div className="flex-[0_0_100%] min-w-0 relative" key={movie.id}>
            <Image
              ...
              height={1080}
            />
            <div className="hidden md:inline absolute top-0 left-0 p-12 bg-transparent w-full h-full bg-gradient-to-r from-gray-900/90 via-transparent to-transparent">
              <div className="h-[65%]" />
              <h2 className="text-3xl font-bold max-w-xl py-4">
                {movie.title}
              </h2>
              <p className=" max-w-lg line-clamp-3">{movie.overview}</p>
            </div>
          </div>
        ))}
      </div>
